...
export default function NewPost() {
  ...
  const { isLoading: isAIImageLoading, mutate: sendAIImageRequest } = useMutation(async () => {
    try {
      const resp = await axios.post("http://localhost:4000/generate-image", { imagePrompt: promptInput });
      if (!resp.data) {
  ...
  const { isLoading: isAIVideoLoading, mutate: sendAIVideoRequest } = useMutation(async () => {
    try {
      ...
      setModalPostVideoUrl(resp.data.imageUrl as string);
    } catch (e) {
      if (e instanceof AxiosError) {
        throw new Error(e.message);
      }
  ...
  const { isLoading: isAICaptionLoading, mutate: sendAICaptionRequest } = useMutation(async () => {
    try {
      const resp = await axios.post("http://localhost:4000/generate-caption", { image: modalPostImgUrl });
      if (!resp.data) {
        throw new Error('Something went wrong!');
      }
  ...
  const { isLoading: isAITextLoading, mutate: sendAITextRequest } = useMutation(async () => {
    try {
      ...
      setModalMediaCaption(resp.data)
    } catch (e) {
      if (e instanceof AxiosError) {
        throw new Error(e.message);
      }
  ...
  const saveAIPrompt = () => {
    ...
    if (modalMediaCaption) {
      setPostContent(prev =>
        prev == placeholder ? modalMediaCaption : prev + "\n" + modalMediaCaption
      );
    }
    setIsModalOpen(false);
  }
  ...
  function createPost(e: React.FormEvent) {
    e.preventDefault();
    mutate({ userId, content: postContent });
  }
    ...
    <div className="py-4 my-3 glass-effect mx-3 rounded-md">
      <form onSubmit={createPost} className="ml-10">
        ...
        {postImgUrl && <img
          ...
          alt="Post"
        />}
        {/* <MediaPlayer title="Video" className="h-24 aspect-video" src="https://files.vidstack.io/sprite-fight/720p.mp4">
          <MediaProvider />
        </MediaPlayer> */}
      ...
      {isModalOpen ?
        <div
          ...
          className="p-4 absolute left-1/2 top-1/4 -translate-x-1/2 -translate-y-1/4 rounded-md shadow-lg w-1/2 overflow-y-auto overflow-x-hidden no-scrollbar"
          style={{
        ...
        >
          ...
          </div>
          {
            ...
            <div className="px-4 pb-1 border-l border-gray-400 my-8 ml-10">
              ...
              {
                (modalPostImgUrl || modalPostVideoUrl) ?
                  <button onClick={() => {
